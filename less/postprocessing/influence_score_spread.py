...
def plot_histogram(df, column, bucket_width=0.05, output_base="histogram", output_dir="."):
    ...
    plt.hist(df[column], bins=bins, color="dodgerblue")
...
def plot_score_with_percentiles(df, column="score", output_base="score_plot_percentiles", output_dir="."):
    ...
    # plt.title(f"Top-k Percentage Influence Scores")
    # plt.legend(loc="best")
    plt.grid(True)

    # Customize x-ticks to show every 10% from 0% to 100%
    plt.xticks(np.arange(0, 101, 10))
    plt.yticks(np.arange(0, 0.15, 0.02))  # ticks from 0 to 0.14 (0.14 included)
...
if __name__ == "__main__":
    # Load DataFrame
    csv_file_path = "less/selected_data/saved/llama3_2_1B_instruct/truthful_qa/sorted.csv"
