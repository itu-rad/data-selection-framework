...
class InferenceRecipe:
    ...
    def generate(self, cfg: DictConfig):
        ...
        full_text = self._tokenizer.decode(generated_tokens[0])
        answer = full_text[len(cfg.prompt.user):].strip()
...
def load_prompts(path):
    with open(path, "r", encoding="utf-8") as f:
        prompts = json.load(f)  # expects a JSON array of strings
        return prompts
    

def main() -> None:
    cfg = OmegaConf.load("less/postprocessing/custom_generation_config.yaml")
    config.log_config(recipe_name="InferenceRecipe", cfg=cfg)

    results = []
    for prompt in load_prompts("less/postprocessing/10prompts.json"):
        cfg.prompt.user = prompt
        recipe = InferenceRecipe(cfg=cfg)
        recipe.setup(cfg=cfg)
        answer = recipe.generate(cfg=cfg)
        print(answer)
        results.append({
            "prompt": prompt,
            "answer": answer.strip()
    ...
    with open("less/postprocessing/10answers_full3.txt", "w", encoding="utf-8") as f:
        f.write(cfg.checkpointer.checkpoint_dir + "\n"*3)
